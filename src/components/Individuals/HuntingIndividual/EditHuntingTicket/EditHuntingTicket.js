...
import EditHuntingTicketForm from './EditHuntingTicketForm';

import { editHuntingLicense } from '../../../../store/huntingLicense/actions';
...
const EditHuntingTicket = ({ data, classs, icon }) => {
  const [initValues, setInitValues] = React.useState(INIT_VALUES);
  ...
  const editItem = (values) => {
    dispatch(editHuntingLicense(values))
      .then((data) => {
        toast.success(data);
        closeModal();
      })
  ...
  return (
    <>
      ...
      <Modal show={isModalOpen} onHide={closeModal} size="lg" className="modal-huntingTicket">
        ...
        <Modal.Body>
          <EditHuntingTicketForm onSubmit={editItem} initialValues={initValues} />
        </Modal.Body>
        <Modal.Footer>
...
export default EditHuntingTicket;
